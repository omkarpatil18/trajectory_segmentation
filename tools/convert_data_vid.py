...
video_path = os.path.join(DATA_DIR, "dataset/videos")
action_path = os.path.join(DATA_DIR, "dataset/actions")
gripper_path = os.path.join(DATA_DIR, "dataset/gripper")
...
def save_action(hash_id, action):
    ...
    np.savez(
        f"{action_path}/{hash_id}",
        features=np.array(action, dtype="float32")[:: (2 * fps)],
    )
...
def save_gripper(hash_id, gripper):
    ...
    np.savez(
        f"{gripper_path}/{hash_id}",
        features=np.array(gripper, dtype="float32")[:: (2 * fps)],
    )
...
def map_to_csv(data):
    ...
    for task in tqdm(tasks, desc="Converting tasks"):
        ...
        for variation in variations:
            episodes = [
                name for name in os.listdir(f"{data_path}/{task}/{variation}/episodes/")
            ]
            for episode in episodes:
                ...
                temp = {
                    "data_path": temp_path,
                    "auto_id": hashlib.md5(temp_path.encode()).hexdigest(),
                }
                writer_map.write(temp)
                writer = jsonlines.open(
                    f"{json_path}/{temp['auto_id']}.jsonl", mode="w"
                )
                # print (temp)

                # copy images to centralized video folder with md5 after converting to video
                duration = save_video(
                    f"{temp_path}/front_rgb", temp["auto_id"], "front"
                )
                        ...
                        try:
                            query = {
                                ...
                                "relevant_clip_ids": [
                                    int(i / 2)
                                    for i in range(
                                        (int(change_point[i][0]) // 2) * 2,
                                        int(change_point[i][1]),
                                        2,
                                    )
                                ],
                            }
                            query["qid"] = hashlib.md5(
                                query["query"].encode()
                            ).hexdigest()
                            query["saliency_scores"] = [
                                [4, 4, 4]
                                for i in range(len(query["relevant_clip_ids"]))
                            ]
                            writer.write(query)
