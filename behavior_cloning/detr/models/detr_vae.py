...
class DETRVAE(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "pos_table", get_sinusoid_encoding_table(num_queries + 1, hidden_dim)
        )
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None, task_emb=None):
        ...
        if is_training:
            ...
            encoder_input = torch.cat(
                [cls_embed, action_embed], axis=1
            )  # (bs, seq+1, hidden_dim)

            encoder_input = encoder_input.permute(1, 0, 2)  # (seq+1, bs, hidden_dim)
            # do not mask cls token
            cls_is_pad = torch.full((bs, 1), False).to(
                qpos.device
            )  # False: not a padding
            ...
            for cam_id, cam_name in enumerate(self.camera_names):
                if self.use_film:
                    features, pos = self.backbones[0](
                        image[:, cam_id], task_emb=task_emb
                    )  # HARDCODED
                else:
...
class CNNMLP(nn.Module):
    def __init__(self, backbones, state_dim, camera_names):
        ...
        if backbones is not None:
            ...
            self.mlp = mlp(
                input_dim=mlp_in_dim, hidden_dim=1024, output_dim=8, hidden_depth=2
            )
        else:
